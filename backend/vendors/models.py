...
class VendorMarketplaceItem(models.Model):
    """Marketplace item for vendors selling fabrics."""
    vendor = models.ForeignKey(
        'vendors.VendorProfile',
        on_delete=models.CASCADE,
        related_name='marketplace_items'
    )
    title = models.CharField(max_length=200, help_text="What fabric are you selling?")
    fabric_type = models.CharField(
        max_length=100,
        help_text="e.g., Ankara, Lace, Cotton, Silk"
    )
    price = models.DecimalField(max_digits=10, decimal_places=2)
    quantity = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        help_text="Available yards/meters/pieces"
    )
    description = models.TextField(
        max_length=500,
        blank=True,
        help_text="Describe the fabric – e.g., 'Soft Ankara, perfect for party wear. No stains.'"
    )
    images = models.ManyToManyField(
        CloudinaryImage,
        blank=True,
        related_name='vendor_marketplace_items'
    )
