...
from .models import CloudinaryImage, FabricItem, VendorMarketplaceItem, VendorProfile
from .serializers import (
    CloudinaryImageSerializer,
    FabricItemSerializer,
    PriceUpdateSerializer,
    StockAdjustmentSerializer,
    VendorMarketplaceItemSerializer,
...
class VendorMarketplaceViewSet(viewsets.ModelViewSet):
    queryset = VendorMarketplaceItem.objects.select_related('vendor', 'vendor__user').prefetch_related('images')
    serializer_class = VendorMarketplaceItemSerializer
    permission_classes = [permissions.IsAuthenticated]
    ...
    def perform_create(self, serializer):
        user = self.request.user
        if user.role != 'vendor':
            raise PermissionDenied('Only vendors can list items.')
        vendor_profile = VendorProfile.objects.filter(user=user).first()
        if not vendor_profile:
            raise PermissionDenied('Create your vendor profile first.')
        ...
        item = serializer.save(vendor=vendor_profile)
        if images:
